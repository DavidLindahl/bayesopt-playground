...
class CNN(nn.Module):
    def __init__(
        ...
        self.last_conv_layer = conv_nodes_2
        ...
        self.fc1 = nn.Linear(self.last_conv_layer * 7 * 7, fc_nodes)
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x = x.view(-1, self.last_conv_layer * 7 * 7)  # Flatten the tensor for the fully connected layer
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
